...
def estimate_data_average_treatment_effects(
    ...
    control_label: Any, # control
    method: Literal["mean", "perturbseq", "adtseq"],
    ...
    估计每种扰动(treatment)相对于控制组(control)的平均处理效应(Average Treatment Effect, ATE),即在表达矩阵上各个基因的扰动差异。
    """
    if compute_fdr:
        raise NotImplementedError

    valid_methods = ["mean", "perturbseq", "adtseq"]
    ...
    if method =='adtseq':
        X_control = np.log1p(X_control)
        X_control = X_control - X_control.mean(axis=1, keepdims=True)
    ...
    for alt_label in tqdm(alt_labels):
        ...
        if method == "adtseq":
            X_alt = np.log1p(X_alt)
            X_alt = X_alt - X_alt.mean(axis=1, keepdims=True)
...
def estimate_model_average_treatment_effect(
    ...
    n_phenos_2 = model_samples["x_2"].shape[-1]
    ...
    X_adt_control_sum = torch.zeros((1, n_phenos_2), device=device)
    
    X_alt_sums = torch.zeros((dosages_alt.shape[0], n_phenos), device=device)
    X_adt_alt_sums = torch.zeros((dosages_alt.shape[0], n_phenos_2), device=device)
    ...
    for i in range(0, n_particles, batch_size):
        ...
        X_adt_control = model_samples["x_2"].squeeze(1)
        X_adt_control = torch.log1p(X_adt_control)
        X_adt_control = X_adt_control - X_adt_control.mean(axis=1, keepdims=True)

        X_control_sum[0] += X_control.sum(0)
        X_adt_control_sum[0] += X_adt_control.sum(0)
        ...
        for t_idx in tqdm(range(dosages_alt.shape[0])):
            ...
            X_adt_curr = model_samples["x_2"].squeeze(1)

            X_adt_curr = torch.log1p(X_adt_curr)
            X_adt_curr = X_adt_curr - X_adt_curr.mean(axis=1, keepdims=True)
            
            X_alt_sums[t_idx] += X_curr.sum(0)
            X_adt_alt_sums[t_idx] += X_adt_curr.sum(0)
    ...
    X_adt_control_mean = X_adt_control_sum / n_particles
    X_adt_alt_means = X_adt_alt_sums / n_particles
    average_adt_effects = (X_adt_alt_means - X_adt_control_mean).detach().cpu().numpy()

    return average_effects, average_adt_effects

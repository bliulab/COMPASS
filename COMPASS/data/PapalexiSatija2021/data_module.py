...
class BasePapalexiSatijaDataModule(PerturbationDataModule):
    def __init__(
        ...
        self.adata.obs['pert_gene'] = self.adata.obs["perturbation"].str.replace(r'g\d+$', '', regex=True).fillna('control')
        targets = list(set(self.adata.obs['pert_gene'].astype(str).unique()))
        ...
        treatment_labels = self.adata.obs["pert_gene"].astype(str)
...
class PapalexiSatijaOODCombinationDataModule(BasePapalexiSatijaDataModule):
    def __init__(
        self,
    ...
    def _get_split_labels(
        ...
        combo_guide_identities = np.sort(
            obs[obs["nperts"] >= 1]["treatment"].astype(str).unique()
        )
...
class PapalexiSatijaDataEfficiencyDataModule(BasePapalexiSatijaDataModule):
    def __init__(
        self,
...
def build_guide_gene_matrix(adata: anndata.AnnData) -> np.ndarray:
    ...
    for guide_id in guide_ids:
        ...
        genes = [gene.rsplit('g', 1)[0] for gene in guide_id.split(';')]
    ...
    # # Step 5: Create DataFrame for better usability (optional)

...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None, mod: Optional[str] = None, guide_id: Optional[str] = None) -> Dict:
        """
        ...
        guide_id: for Frang dataset is sgRNA; for papa dataset is guide_id
        """
        ...
        if mod=="RNA":
            if guide_id =="sgRNA":
                sgRNA_genes = list(set(adata.obs[guide_id].str.replace(r'_\d+$', '', regex=True).dropna().unique()))
                if "nan" in sgRNA_genes:
                    sgRNA_genes.remove("nan")            
            elif guide_id == "guide_id":
                sgRNA_genes = list(set(adata.obs[guide_id].str.replace(r'g\d+$', '', regex=True).dropna().unique()))
                if "control" in sgRNA_genes:
                    sgRNA_genes.remove("control")
            # 在 adata.var 中添加一个列：是否来自 sgRNA
            adata.var["from_sgRNA"] = adata.var["gene_name"].isin(sgRNA_genes)
        else:
        ...
        if self.filter_gene_by_counts:
            ...
            raw_X = adata.X# if self.use_key is None else adata.layers[self.use_key]
            raw_X = raw_X.toarray() if issparse(raw_X) else raw_X
            gene_counts = raw_X.sum(axis=0)

            keep_gene_mask = (gene_counts >= self.filter_gene_by_counts) | adata.var["from_sgRNA"].values
            n_before = adata.shape[1]
            adata._inplace_subset_var(keep_gene_mask)
            n_after = adata.shape[1]
            print(f"Filtered genes: {n_before - n_after} removed, {n_after} kept.")
        ...
        if self.subset_hvg:
            ...
            if mod=="RNA":
                ...
                keep_hvg = adata.var["highly_variable"]
                keep_sgRNA = adata.var["from_sgRNA"] if "from_sgRNA" in adata.var.columns else False

...
class PerturbationPlatedELBOLossModule(torch.nn.Module):
    ...
    def forward(
        ...
        guide_dists, guide_samples = self.guide( 
            X=embed,#X
            D=D,
            condition_values=condition_values, ## newly added
            n_particles=n_particles,
        )

        for k, v in guide_samples.items():
            condition_values[k] = v #guide 采样到的隐变量做condition value

        model_dists, model_samples = self.model( # 生成过程
            D=D,
            condition_values=condition_values,
            n_particles=n_particles,
        ) # model_samples['x'] model_samples['x_2'] model_samples['z']
    ...
    def loss(
        ...
        loss_terms["reconstruction_2"] = model_dists["p_x_2"].log_prob(X2).sum(-1)

        for k in guide_dists.keys():
            var_key = k[2:]  # drop 'q_' guide网络生成的各个隐变量的后验分布

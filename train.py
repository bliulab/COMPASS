...
from COMPASS.models.utils.lightning_callbacks import (
    GradientNormTracker,
...
def add_data_info_to_config(config: Dict, data_module: PerturbationDataModule):
    ...
    config["model_kwargs"]["n_treatments"] = data_module.get_d_var_info().shape[0]#264
    n_phenos, n_phenos_2 = data_module.get_x_var_info()
    config["model_kwargs"]["n_phenos"] = n_phenos.shape[0]
    config["model_kwargs"]["n_phenos_2"] = n_phenos_2.shape[0]
    # for sams_vae:
    config["guide_kwargs"]["n_phenos"] = data_module.get_embed_var_info().shape[1]
